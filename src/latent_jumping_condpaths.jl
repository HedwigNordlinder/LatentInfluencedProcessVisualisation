...
conditional_path_states = Vector{Vector{LatentJumpingState}}()
...
X0_samples = sampleX0(simulations)
X1_samples = sampleX1(simulations)
X0 = LatentJumpingState(ContinuousState(copy(X0_samples)), DiscreteState(3,fill(2,simulations)), ContinuousState(copy(X0_samples)))
X1 = LatentJumpingState(ContinuousState(copy(X1_samples)), DiscreteState(3,fill(2,simulations)), ContinuousState(copy(X1_samples)))
endpoint_conditioned_sample(X0, X1, P, fill(0.0f0, length(simulation_times)), simulation_times; tracker = conditional_path_tracker, ϵ=1e-2)
plot_trajectories_conditional(conditional_path_times, conditional_path_states, n_plot=5000, alpha=0.01, extract_state = x -> x.combined_state.state[1,:])